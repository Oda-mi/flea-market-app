...
## 追加実装機能
※本機能は、追加機能を実装する過程で案件シートのUIデザイン要件を再現しつつ、<br>
...
- 画面上部には「〇〇さんとの取引画面」という表示を残し、取引相手を常に把握できるように設定

これにより、チャットの履歴量に関わらず、<br>
ユーザーがスムーズに操作できるUXを意識した取引チャット画面としています


**2. メッセージ送信時の画像選択状態表示**

ユーザーストーリーID【US002】<br>
「ユーザーは取引チャットの投稿をすることができる」<br>

取引チャット機能で画像投稿ができること自体は要件にありましたが、<br>
画像選択時の挙動については明記がされていませんでした<br>

そこで、ユーザーにわかりやすいUXを意識し、以下の仕様を設計・実装しました<br>

- 画像を選択すると、画面上に「※画像が選択されています」と表示
- 「取消」ボタンを押すと選択を解除し、「※画像が選択されています」は非表示

これにより、ユーザーが画像の選択状態をひと目で確認できる取引チャット画面としています


**3. 取引チャット編集・削除操作のUX改善（US003）**
...
そこで、ユーザーが直感的に操作できるUXを意識し、以下の仕様を設計・実装しました
...
ユーザーが迷わず直感的に操作できる取引チャット画面としています


**4. 取引完了後のチャット画面UX改善**

本ユーザーストーリーでは、取引評価を完了することで取引が完了する仕様となっていますが、<br>
取引完了後の取引チャット画面の挙動については、機能要件に明記されていませんでした<br>
