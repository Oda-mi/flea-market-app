                ...
                @if ($averageRating > 0)
                    <div class="profile__rating">
                        @for ($starNumber = 1; $starNumber <= 5; $starNumber++)
                            <span
                                class="star {{ $starNumber <= $averageRating ? 'star--active' : 'star--inactive' }}">
                                ★
                            </span>
                        @endfor
                    </div>
    ...
    <div class="nav">
        <div class="nav__tabs">
            <a
                ...
                class="nav__tab {{ $tab === 'sell' ? 'nav__tab--active' : '' }}">
                出品した商品
            </a>
            <a
                href="{{ route('mypage.index', ['page' => 'buy']) }}"
                class="nav__tab {{ $tab === 'buy' ? 'nav__tab--active' : '' }}">
                購入した商品
            </a>
            <a
                href="{{ route('mypage.index', ['page' => 'trading']) }}"
                class="nav__tab {{ $tab === 'trading' ? 'nav__tab--active' : '' }}">
                取引中の商品
        ...
        <div class="product__list">
            @forelse ($tab === 'trading' ? $transactions : $itemsToShow as $itemOrTransaction)
                    ...
                    <a href="{{ route('transactions.show', ['id' => $itemOrTransaction->id]) }}" class="product__list-link">
                        <div class="product__item">
                            <div class="product__image-wrapper">
                                <img
                                    src="{{ asset('storage/images/' . $itemOrTransaction->item->img_url) }}"
                                    alt="{{ $itemOrTransaction->item->name }}">
                ...
                @else
                    ...
                    <div class="product__item">
                        <img
                            src="{{ asset('storage/images/' . $itemOrTransaction->img_url) }}"
                            alt="{{ $itemOrTransaction->name }}">
